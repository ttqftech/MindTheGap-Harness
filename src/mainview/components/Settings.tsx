...
import { z } from "zod";
import type { ModelProvider, ModelConfig, AppMode } from "../store";
...
function ModelsTab() {
    ...
    const handleSave = () => {
        ...
        // 校验 JSON
        try {
            jsonSchema.parse(p.customParams);
        } catch (e: any) {
            setJsonError("自定义参数必须是合法的 JSON 对象");
            return;
        }
...
function ProviderEditor(props: {
    ...
    const [jsonText, setJsonText] = createSignal(
        JSON.stringify(props.provider.customParams, null, 2) || "{}",
    );

    const update = (patch: Partial<ModelProvider>) => {
        props.onChange({ ...props.provider, ...patch });
    };

    const updateModel = (modelId: string, patch: Partial<ModelConfig>) => {
        const models = props.provider.models.map((m) =>
    ...
    const addModel = () => {
        const id = prompt("模型 ID（如 deepseek-v4-pro）：");
        if (!id) return;
        if (props.provider.models.some((m) => m.id === id)) {
            alert(`模型 ${id} 已存在`);
            return;
        }
        const displayName = prompt("显示名称：") || id;
        update({
            models: [...props.provider.models, { id, displayName, role: "standard" }],
        });
    };

    const removeModel = (modelId: string) => {
        update({
            models: props.provider.models.filter((m) => m.id !== modelId),
        });
    };
    ...
    const validateJson = () => {
        try {
            ...
            if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
                ...
                props.onJsonError("");
                update({ customParams: parsed as Record<string, unknown> });
            }
        } catch (e) {
    ...
    return (
        <div>
            ...
            <div class="setting-group">
                ...
                <select
                    class="select-input"
                    value={props.provider.apiFormat}
                    onchange={(e) => update({ apiFormat: e.currentTarget.value as any })}
                >
                        ...
                        {(model) => (
                            <div style={{
                                ...
                                <input
                                    ...
                                    oninput={(e) => updateModel(model.id, { displayName: e.currentTarget.value })}
                                ...
                                <select
                                    ...
                                    style={{ width: "auto" }}
                                    value={model.role}
                                    onchange={(e) => updateModel(model.id, { role: e.currentTarget.value as any })}
                                >
                                    <option value="standard">标准</option>
                                    <option value="economy">节省</option>
                                </select>
                                <span style={{ fontSize: 11, color: "var(--fontColorMuted)" }}>ID: {model.id}</span>
                                <button
                                    class="icon-btn"
                                    title="删除此模型"
            ...
            <div class="setting-group">
                <div class="setting-group-label">自定义参数</div>
                <div class="setting-group-desc">
                    JSON 对象，会被附加到每次请求的参数中。
                    常见：<code>{"{\"temperature\": 0.7, \"max_tokens\": 2000}"}</code>
                </div>
                <textarea
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: 24 }}>
                <button class="btn-secondary" onclick={props.onSave}>保存</button>
...
const ALL_MODES: AppMode[] = ["默认", "编码", "文件夹浏览总结"];

function ModeConfigTab() {
    const [selectedMode, setSelectedMode] = createSignal<AppMode>(state.currentAppMode);
    const currentConfig = createMemo(() =>
        state.modeConfigs.find((c) => c.name === selectedMode())
    );
    const [jsonText, setJsonText] = createSignal("");
    ...
    const handleSave = () => {
        try {
            ...
            if (parsed.transferableAgents && Array.isArray(parsed.transferableAgents)) {
                actions.updateModeConfig(selectedMode(), parsed.transferableAgents);
                alert("已保存！");
            }
        } catch (e) {
            alert("JSON 格式错误");
    ...
    return (
        <div>
            <div class="setting-group">
                <div class="setting-group-label">选择模式</div>
                <select
                    class="select-input"
                    value={selectedMode()}
                    onchange={(e) => setSelectedMode(e.currentTarget.value as AppMode)}
                >
                    <For each={ALL_MODES}>
            ...
            <div class="setting-group">
                ...
                <div class="setting-group-desc">
                    这个模式运行时，允许通过 "转接" 工具将工作转交给这些 Agent。
                    <br />
                ...
                <div style={{
                    ...
                    <For each={ALL_MODES.filter((m) => m !== selectedMode())}>
                        {(mode) => {
                            const checked = currentConfig()?.transferableAgents.includes(mode) ?? false;
                            return (
                                <label style={{ display: "flex", alignItems: "center", gap: "6px", cursor: "pointer" }}>
                                    <input
                                        ...
                                        onchange={(e) => {
                                            ...
                                            actions.updateModeConfig(selectedMode(), list);
                                        }}
                                    />
                                    <span>{mode}</span>
                                </label>
